...
- `GET /api/v1/reports`: 获取所有对比报告。
- `POST /api/v1/reports`: 保存新的对比报告。
- `DELETE /api/v1/reports/:id`: 删除对比报告。
- `GET /api/v1/messages`: 获取所有留言板条目。
- `POST /api/v1/messages`: 在留言板上发布新留言。
...
### 功能 5：留言板
- **功能描述**：一个供用户发布反馈、问题或见解的社区板块。
- **目的**：促进社区参与，并为定量数据之外的定性讨论提供空间。
- **触发器**：用户导航到“留言板”部分。
- **流程**：查看留言 → 点击“发布留言” → 填写内容和作者 → 提交 → 留言出现在板块上。
- **成功标准**：留言持久化，正确显示作者和时间戳，并可由授权用户删除。

### 功能 6：对比报告
- **功能描述**：将对比结果保存为带有摘要的持久化报告。
- **目的**：记录并分享特定的性能发现，以供将来参考。
- **触发器**：用户在对比视图中点击“保存报告”。
- **流程**：进行对比 → 点击保存报告 → 添加摘要/备注 → 保存 → 报告出现在“报告”列表中。
- **成功标准**：报告链接回原始基准测试，存储摘要，并可从专用列表视图访问。
